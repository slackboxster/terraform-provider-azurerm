...
func objectIdFromMsGraph(ctx context.Context, c *Config) (string, error) {
	...
	msGraphAuth, err := c.GetAuthorizationToken(sender.BuildSender("GoAzureHelpers"), oauthConfig, string(env.MsGraph.Endpoint))
	if err != nil {
		return "", err
	}

	authorizerWrapper, err := auth.NewAutorestAuthorizerWrapper(msGraphAuth.(*autorest.BearerAuthorizer))
	if err != nil {
		return "", err
	}
